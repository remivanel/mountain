...
static void calc_bezier(void);

/* le systeme d'annotations */
int etim_nb_anno=0;
int etim_pos_anno[3];
char etim_text_anno[3][10];
...
static float tmp_coefs[MAX_POINT];
/* variables de dialog */
int *retour_index;
...
{	WINDFORM_VAR *ptr_var = &etim_var;
	...
	{	adr_edited_spline = find_time_spline(selected_spline);
		...
		ptr_var->adr_form[ETIM_INTER].ob_state = SELECTED*tempo_spline.interpolee;
		
		/* preparation des annotations */
		ptr_var->adr_form[ETIM_ANNO].ob_flags = HIDETREE;
		ptr_var->adr_form[ETIM_ANNO+1].ob_flags = HIDETREE;
		ptr_var->adr_form[ETIM_ANNO+2].ob_flags = HIDETREE;
		ptr_var->adr_form[ETIM_ANNO+3].ob_flags = HIDETREE;
		ptr_var->adr_form[ETIM_ANNO+4].ob_flags = HIDETREE;
		ptr_var->adr_form[ETIM_ANNO+5].ob_flags = HIDETREE;
		for(i=0;i<etim_nb_anno;i++)
		{	ptr_var->adr_form[ETIM_ANNO+i*2].ob_flags = 0;
			ptr_var->adr_form[ETIM_ANNO+1+i*2].ob_flags = 0;
			ptr_var->adr_form[ETIM_ANNO+i*2].ob_y = 4 + 255 - etim_pos_anno[i];
			if(etim_pos_anno[i] > 128)		/* FAUT-IL PLACER LE TEXT EN DESSOUS OU AU DESSUS */
				ptr_var->adr_form[ETIM_ANNO+1+i*2].ob_y = 4 + 255 - etim_pos_anno[i] + 10;
			else
				ptr_var->adr_form[ETIM_ANNO+1+i*2].ob_y = 4 + 255 - etim_pos_anno[i] - 14;
			
			strcpy(ptr_var->adr_form[ETIM_ANNO+1+i*2].ob_spec.tedinfo->te_ptext,etim_text_anno[i]);
		}
	...
	{	choix = windial_do(ptr_var, event);
		...
		{	
			...
			case ETIM_MAIN :
				set_point();
				if(tempo_spline.mode==3)
				{	graf_mouse(BUSYBEE,0);
					calc_lagrange();
					objc_draw(etim_var.adr_form, ETIM_MAIN, MAX_DEPTH, x_desk, y_desk, w_desk, h_desk);
					graf_mouse(ARROW,0);
				}else
				if(tempo_spline.mode==4)
				{	graf_mouse(BUSYBEE,0);
					calc_bezier();
					objc_draw(etim_var.adr_form, ETIM_MAIN, MAX_DEPTH, x_desk, y_desk, w_desk, h_desk);
					graf_mouse(ARROW,0);
			...
			case ETIM_OK :
				tempo_spline.interpolee=ptr_var->adr_form[ETIM_INTER].ob_state & SELECTED;
				strcpy( tempo_spline.nom, ptr_var->adr_form[ETIM_NAME].ob_spec.tedinfo->te_ptext);
...
{	int base_x,base_y,i,point;
	...
	if(tempo_spline.mode == 2 || tempo_spline.mode == 3  || tempo_spline.mode == 4)
	...
	if(tempo_spline.mode == 2 || tempo_spline.mode == 1)
		objc_draw(etim_var.adr_form, ETIM_MAIN, MAX_DEPTH, x_desk, y_desk, w_desk, h_desk);
...
{	int xy[4],i,old;
	...
	{	real.g_x = base_x-1;
		...
		set_clip(0,0);
	}else if(tempo_spline.mode==3 || tempo_spline.mode==4)
	{	real.g_x = base_x-1;
		real.g_y = base_y;
		real.g_w = 258;
		real.g_h = 256;
		...
		{	rect_full(base_x+tempo_spline.point[x1-1][0]-1,base_y,tempo_spline.point[x1+1][0]-tempo_spline.point[x1-1][0]+2,256, WHITE);
			deb=x1-1; fin=x1+1;
		}
		vsm_type(vdi_handle,4);
		vsm_height(vdi_handle,6);
		...
		if(deb<0) deb=0;
		fin++;
		if(fin>tempo_spline.nb_points) fin=tempo_spline.nb_points;
		
		for(i=deb;i<fin;i++)
		{	xy[0]=tempo_spline.point[i][0]+base_x;
			xy[1]=255-tempo_spline.point[i][1]+base_y;
			v_pmarker(vdi_handle,1,xy);
		}
		set_clip(0,0);
	}
...
{	int i;
	...
	la_spline->interpolee = FALSE; 
	la_spline->nb_points = 3; 
...
{	float y,li;
	...
	{	tmp_coefs[i] = 1;
		for(j=0 ; j<n ; j++)
		{	if(i!=j)
				tmp_coefs[i] *= (float)(tempo_spline.point[i][0] - tempo_spline.point[j][0]);
		}
	}
	...
	{	y=0;
		...
		{	li=1;
			for(j=0 ; j<n ; j++)
			{	if(i!=j)
					li *= (float)(xc - tempo_spline.point[j][0]);
			}
			li = li/tmp_coefs[i];
			y += (float)tempo_spline.point[i][1] * li;
		}
		if(y<0) y=0;
		if(y>255) y=255;
...
/* Calcul selon methode de bezier */
static void calc_bezier(void)
{	float t,a,b,c,d,tg0,tg1;
	double y;
	int i,n=tempo_spline.nb_points-1,x,l,x0;
	
	for(i=0;i<n;i++)
	{	x0=tempo_spline.point[i][0];
		/* premiere tangente */
		if(i==0)
			tg0 = ((float)(tempo_spline.point[1][1]-tempo_spline.point[0][1]));
		else
		{	tg0 = ((float)(tempo_spline.point[i+1][1]-tempo_spline.point[i-1][1]))/((float)(tempo_spline.point[i+1][0]-tempo_spline.point[i-1][0]));
			tg0 *= tempo_spline.point[i+1][0]-x0;
		}
		/* seconde tangente */
		if(i==n-1)
			tg1 = ((float)(tempo_spline.point[i+1][1]-tempo_spline.point[i][1]));
		else
		{	tg1 = ((float)(tempo_spline.point[i+2][1]-tempo_spline.point[i][1]))/((float)(tempo_spline.point[i+2][0]-x0));
			tg1 *= tempo_spline.point[i+1][0]-x0;
		}
		
		/* Resolution */
		d = tempo_spline.point[i][1];
		a = 2*(d-tempo_spline.point[i+1][1]) + tg0 + tg1;
		b = 3*(tempo_spline.point[i+1][1]-d) - 2*tg0 - tg1;
		c = tg0;
		
		/* trac‚ */
		l=tempo_spline.point[i+1][0]-x0;
		for(x=0;x<=l;x++)
		{	t=(float)x/l;
			y = a*t*t*t + b*t*t + c*t + d;
			if(y>255) y = 255;
			if(y<0) y=0;
			tempo_spline.valeur[x+x0] = (unsigned char)y;
		}
	}
}
...
{	GRECT area,real;
	...
	if(tempo_spline.mode == 1 || tempo_spline.mode == 3  || tempo_spline.mode == 4)	/* manuel */
	...
	if(tempo_spline.mode == 3 || tempo_spline.mode == 4)	/* lagrange ou bezier */
	{	vsm_type(vdi_handle,4);
		vsm_height(vdi_handle,6);
		vsm_color(vdi_handle,LBLUE);
...
{	TIME_SPLINE *la_spline;
	...
	int abscisse1,i,j,n,x0;
	double y,li;
	float y0,y1,max;
	double coef;
	float t,a,b,c,d,tg0,tg1,l;
	...
	{	if(la_spline->interpolee && max_abscisse>256)		/* CALCUL TRES FIN MAIS COMPLEXE */
		{	base = ((long)abscisse1*max_abscisse)/255;
			max =  (float)max_abscisse/255;
			y0 = ((float)la_spline->valeur[abscisse1]*max_ordonee)/255;
			y1 = ((float)la_spline->valeur[abscisse1+1]*max_ordonee)/255;
			y = y0 + ((y1-y0)*(float)(time-base))/max;
			ordonee=(long)y;
		}else
	...
	}else if(la_spline->mode == 3) 	/* MODE LAGRANGE */
	{	if(la_spline->interpolee)		/* CALCUL TRES FIN MAIS COMPLEXE */
		{	
			n = la_spline->nb_points;
			...
			{	coef = 1;
				for(j=0 ; j<n ; j++)
				{	if(i!=j)
						coef *= (double)(max_abscisse*(la_spline->point[i][0] - la_spline->point[j][0])/255);
				}
				
				li=1;
				for(j=0 ; j<n ; j++)
				{	if(i!=j)
						li *= (double)(time - (max_abscisse*(long)la_spline->point[j][0])/255);
				}
				li = li/coef;
				y += (double)la_spline->point[i][1] * li * (double)max_ordonee / 255;
			}
			if(y<0) y=0;
			if(y>max_ordonee) y=max_ordonee;
			ordonee=(long)y;
		}else								/* mode rapide, sans calcul complexe */
			ordonee = ((long)(la_spline->valeur[abscisse1]) * max_ordonee) / 255;

	}else if(la_spline->mode == 4)	/* MODE BEZIER */
	{	if(la_spline->interpolee)		/* CALCUL TRES FIN MAIS COMPLEXE */
		{	n = la_spline->nb_points-1;
			for(i=0;i<n;i++)
			{	if(abscisse1>=la_spline->point[i][0] && abscisse1<la_spline->point[i+1][0])
					break;
			}
			x0=la_spline->point[i][0];
			/* premiere tangente */
			if(i==0)
				tg0 = ((float)(la_spline->point[1][1]-la_spline->point[0][1]));
			else
			{	tg0 = ((float)(la_spline->point[i+1][1]-la_spline->point[i-1][1]))/((float)(la_spline->point[i+1][0]-la_spline->point[i-1][0]));
				tg0 *= la_spline->point[i+1][0]-x0;
			}
			/* seconde tangente */
			if(i==n-1)
				tg1 = ((float)(la_spline->point[i+1][1]-la_spline->point[i][1]));
			else
			{	tg1 = ((float)(la_spline->point[i+2][1]-la_spline->point[i][1]))/((float)(la_spline->point[i+2][0]-x0));
				tg1 *= la_spline->point[i+1][0]-x0;
			}
		
			/* Resolution */
			d = la_spline->point[i][1];
			a = 2*(d-la_spline->point[i+1][1]) + tg0 + tg1;
			b = 3*(la_spline->point[i+1][1]-d) - 2*tg0 - tg1;
			c = tg0;
		
			/* Calcul */
			l= max_abscisse * (float)(la_spline->point[i+1][0]-x0)/255;
			t=(float)(time-(max_abscisse*(float)x0)/255)/l;
			y = a*t*t*t + b*t*t + c*t + d;
			if(y>255) y = 255;
			if(y<0) y=0;
			ordonee = ((long)y*max_ordonee)/255;
		}else								/* mode rapide, sans calcul complexe */
			ordonee = ((long)(la_spline->valeur[abscisse1]) * max_ordonee) / 255;
	}
	return ordonee;
